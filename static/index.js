...
(function(w) {
    ...
    function shareText(score) {
        ...
        if (mode === MODE_ENDLESS) {
            //无尽模式嘲讽,力度轻点罢(?)
            if (score <= 5) return '太拉了';
            if (score <= 8) return '您还是少冲点罢';
            if (score <= 10)  return '就这?就这?';
            return '您再去练练?';
        } else {
            //笑,都给我笑
            if (score <= 5) return '太拉了';
            if (score <= 8) return '您还是少冲点罢';
            if (score <= 10)  return '就这?就这?';
            if (score <= 49) return '逊诶(恼)';
            if (score <= 99) return '手指还没我橡皮灵活(乐)';
            if (score <= 149) return '您?';
            if (score <= 299) return '人?';
            if (score <= 114514) return '先辈传人(喜)';
            if (score <= 1919810) return '先辈继承人(乐)';
            if (score <= 1145141919810) return '不要欺负你的手机识别不了你是不是神(?)';
            return '植粉马斯内';
        }
    }
    ...
    function initSetting() {
        ...
        document.getElementsByTagName("title")[0].innerText = cookie("title") ? cookie("title") : "植 粉 被 撅 力";
        if (cookie("keyboard")) {
